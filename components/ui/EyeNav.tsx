...
export const EyeNav = () => {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 flex items-end justify-between p-4">
      <div className="pointer-events-auto select-none rounded-full bg-white/5 px-3 py-1 text-xs text-white/75 backdrop-blur-md">
        {idx + 1} / {total}
      </div>
      <button
        type="button"
        className="pointer-events-auto rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/20"
        onClick={handleNext}
        aria-label="Next Eye"
      >
        Next
